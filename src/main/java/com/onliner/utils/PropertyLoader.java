...
import com.onliner.exception.NoPropertiesException;
...
public class PropertyLoader {
    ...
    public static String returnConfigValue(final String property) {
        ...
        try (InputStream propFileInpStream = PropertyLoader.class.getClassLoader()
            ...
            LOGGER.debug(MessageFormat.format("Getting {0} inside file environment config.properties: {2}",
                    property, properties.getProperty(property)));

            return properties.getProperty(property);
        } catch (IOException e) {
            LOGGER.error(MessageFormat.format("Property {0} or directory not found inside of configuration files",
                    property), e);
